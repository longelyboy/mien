...
def order(robot_id, side, exchange, market, amount, direction):
    ...
    try:
        ...
        orderInfo = exchange.fapiPrivatePostOrder({
            'symbol': market,
            'type': 'MARKET',
            'side': side_type,
        ...
        order_info = {}
        order_info['order_id'] = orderInfo['orderId']  # 订单id
        time.sleep(1)  # 延迟1秒等交易完成

        print('订单id:', order_info['order_id'])
        order_log = getOrderInfo(exchange, market, order_info['order_id'])

        print('订单记录:', order_log)

        errorNum = 0
        while order_log['code'] == 1 and order_log['data']['status'] != 'FILLED':
            time.sleep(0.5)
            order_log = exchange.getOrderInfo(exchange, market, order_info['order_id'])

            errorNum += 1

            print("为获取到订单信息，重新获取订单：", order_info['order_id'])
            # if errorNum >= 5:
        ...
        if order_log['code'] == 1:
            order_info['deal_money'] = order_log['data']['origQty']  # 委托数量张数
            order_info['deal_price'] = order_log['data']['price']  # 委托价格
            order_info['trade_avg_price'] = order_log['data']['avgPrice']  # 成交均价
            order_info['trade_volume'] = order_log['data']['cumQuote']  # 成交量（U）
            order_info['order_status'] = order_log['data'][
                'status']  # 订单状态	canceled：撤单成功 、 live：等待成交 、 partially_filled：部分成交、filled：完全成交
            order_info['deal_fee'] = float(order_log['data']['cumQuote']) * float(sxf_rate)  # 总手续费
...
def getAllTick(exchange, platform):
    ...
    try:
        ...
        # 获得所有币种的行情
        ticker_data = exchange.fapiPublicGetTicker24hr()

        # print(type(ticker_data))
        if isinstance(ticker_data, list):
            for ticker_item in ticker_data:

                # print(ticker_item['symbol'])
                # print('platform：',platform)
                if platform not in all_ticker:
                    all_ticker[platform] = {}
                all_ticker[platform][ticker_item['symbol']] = ticker_item
    ...
    try:
        ...
        if platform in all_ticker:
            if market in all_ticker[platform]:
                print(platform, ' ', market, ' price:', all_ticker[platform][market]['lastPrice'])
                return all_ticker[platform][market]['lastPrice']
        else:
...
def setAccountLeverage(exchange, market):
    ...
    try:
        result = exchange.fapiPrivatePostLeverage({
            'symbol': market,
            'leverage': 10
        })

        ret['code'] = 1
        ret['data'] = result['leverage']
        ret['msg'] = 'success'
        return ret
...
def getAccountConfig(exchange):
    ...
    try:
        ...
        ret['code'] = 1
        ret['data'] = str(result['dualSidePosition'])
        ret['msg'] = 'success'
        return ret
...
def verifyMode(exchange):
    ...
    if smode['code'] == 1:
        ...
        if smode['data'] == 'false' or smode['data'] == 'False':
...
def getOrderInfo(exchange, market, order_id):
    ...
    try:
        result = exchange.fapiPrivateGetOrder({
            'symbol': market,
            'orderId': order_id
        })
        ret['code'] = 1
...
# 获取手续费率
def getSxf(exchange, symbol):
    ret = {}

    try:
        result = exchange.fapiPrivateGetCommissionRate({
            'symbol': symbol
        })

        ret['code'] = 1
        ret['data'] = result['takerCommissionRate']
        ret['msg'] = 'success'
        return ret
...
def onTick(robot_id, market_info, api_info):
    ...
    # 设置下单杠杆倍数
    changeLevel = setAccountLeverage(exchange, market)

    print(changeLevel)
    if changeLevel['code'] == 0:
        thread_alive[robot_id] = 0
        print('初始化杠杆倍数失败,robot:' + str(robot_id))
        return

    # aa = getAccountConfig(exchange)
    changeMode = verifyMode(exchange)

    # print(changeMode)
    if changeMode['code'] == 1:
        print('检测到binance交易所:%s 交易模式不是多空，重置成功' % market)
    else:
        thread_alive[robot_id] = 0
    ...
    if values_str:
        ...
        # 做多：合約數量 *  (標記價格 - 訂單價格) 如果>0  盈利 <0 亏损
        # 做空：合約數量 *  (订单价格 - 标记价格) 如果>0  盈利 <0 亏损
        ...
        # 计算初始保证訂單的預估價格 * 合約數量 / 槓桿

        margin_money = base_price * float(values_dict['deal_amount']) / 10

        updateRevenue(robot_id, revenue)

        if revenue < 0:

            zhisun = abs(revenue) * 100 / margin_money
            ...
            if zhisun >= sl_trigger_price:
                ...
                if rst['code'] == 1:
                    ...
                    trade_volume = rst['data']['trade_volume']

                    deal_money = price * float(deal_amount) / 10
        ...
        if is_clean == 1:
            ...
            if rst['code'] == 1:
                ...
                # 计算初始保证金  开仓保证金=面值*张数*最新标记价格／杠杆
                # deal_money = rst['data']['trade_volume']
                ...
                # 收益
                if direction == 1:
                    revenue = float(deal_amount) * (float(price) - base_price)
                else:
                    revenue = float(deal_amount) * (base_price - float(price))
        ...
        print('当前收益率：', revenue * 100 / margin_money)
        # 收益率=收益/开仓所需保证金
        if tick_price > base_price:
            ...
            if direction == 1:
                if trend_side == 0 or trend_side == 2:
                    ...
                    up_rate = revenue * 100 / margin_money
                ...
                if trend_side == 1:
                    ...
                    if tick_price > up_price:
                        ...
                        down_rate = float(values_dict['deal_amount']) * down_num * 100 / margin_money
                        ...
                        if down_rate >= stop_profit_callback_rate:
                            ...
                            if rst['code'] == 1:
                                ...
                                insertLog(platform, robot_id, uid,
                                          u"开多卖出：成交总价 %s %s ,成交均价 %s %s,成交数量(张) %s %s,手续费 %s" % (
                ...
                if trend_side == 0 or trend_side == 1:
                    ...
                    down_rate = abs(revenue) * 100 / margin_money  # 价格下降  做多  为负数
                ...
                if trend_side == 2:
                    ...
                    if tick_price > down_price:
                        ...
                        up_rate = float(values_dict['deal_amount']) * up_num / margin_money
                        ...
                        if up_rate >= cover_callback_rate:
                            ...
                            if rst['code'] == 1:
                                ...
                                values_dict['trade_volume'] = float(
                                    values_dict['trade_volume']) + float(trade_volume)
        ...
        if rst['code'] == 1:
            ...
            values_dict = {'first_order_price': price, 'base_price': price, 'up_price': 0, 'down_price': 0,
                           'trend_side': 0,
                           'order_count': 1, 'deal_amount': deal_amount, 'deal_money': deal_money,'trade_volume': trade_volume, 'order_finish': 0,
            ...
            disableRobot(robot_id)
            updateMsg(robot_id, u'首单开单失败')
            insertLog(platform, robot_id, uid, u'器人已经关闭-首单开单失败')
    # return
...
def loadStrategyConfig(cur):
    ...
    markets_count = cur.execute(
        "select id,platform,market,market_name,type,stock,money,contract_size from jl_spot_market where status = 1 and platform = 'binance'")
    ...
    api_count = cur.execute(
        "select id,uid,platform,api_key,secret_key,passphrase from jl_third_api where status = 1 and platform = 'binance'")
    ...
    # print(api_list)
    cur.execute("SELECT * from jl_quant_robot where status = 1 and type = 2 and platform = 'binance'")
    robots = cur.fetchall()
    if robots:
        for robot in robots:
