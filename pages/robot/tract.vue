<template>
  <div>
    ...
    <van-row class="block1">
      <van-col :span="12">
        {{ $t('first_trade_num') }} <span>{{ robot.first_order_value }}</span>
      </van-col>
      <van-col :span="12">
      ...
      <van-col :span="12">
        {{ $t('direction') }} <span v-if="robot.direction == 1" style="color:green">{{ $t('directionbuy') }}</span>
        <span v-else style="color:red">{{ $t('directionsell') }}</span>
      </van-col>
    </van-row>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      apiAccountBalance: 'authorize/apiSwapAccountBalance',
      publicTicker: 'robot/publicSwapTicker'
    }),
    goEdit () {
      this.$router.push({
        name: 'robot-contract',
